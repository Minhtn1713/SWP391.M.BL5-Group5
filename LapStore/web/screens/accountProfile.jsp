                            ...
                            <form method="POST" action="account-profile?status=profile">
                                ...
                                <c:if test="${not empty error6}"><h3 style="color: red; text-align: center; width: 100%; margin: auto">${error6}</h3><br/></c:if>
                                    ...
                                    <select name="gender" class="user-inf">
                                        <option value="0" ${param.gender == '0' ? 'selected' : (user.gender == 'false' ? 'selected' : '')}>Male</option>
                                    <option value="1" ${param.gender == '1' ? 'selected' : (user.gender == 'true' ? 'selected' : '')}>Female</option>
                                </select><br/>
                                <p>Phone</p>
                                <input name="phone" type="number" class="user-inf" value="${param.phone != null ? param.phone : user.phone}" required/><br/>
                                <c:if test="${not empty error3}"><h3 style="color: red; text-align: center; width: 100%; margin: auto">${error3}</h3><br/></c:if>
                                    <p>Email</p>
                                    <input name="email"type="email" class="user-inf" value="${param.email != null ? param.email : user.email}" required/><br/>  
